...
// times out on its own end after 100s -- that budget covers any web searches the AI runs
// too, not just the model call itself; a real search-then-reask round trip against live
// APIs is meaningfully slower than the instant mock responses this was first tested
// against, so the original 60s budget cut off legitimate multi-round lookups) -- slightly
// longer than that, so the server's own clearer timeout message wins in the normal case
// and this is just a last resort.
async function fetchAiWithTimeout(url, options) {
  ...
  try {
    ...
    if (e.name === 'AbortError') {
      return { ok: false, data: { error: 'This is taking far longer than expected (over 120s) and was cancelled on this end. The AI service may be overloaded — try again in a bit, or try a shorter description.' } };
    }
...
async function extractPartsWithAI() {
  ...
  if (aiAttachedFile) {
    ...
    startAiTimer(isImage ? '15-25s for images, up to 100s if it needs to search the web' : '10-20s for documents, up to 100s if it needs to search the web');
    ...
    startAiTimer('5-10s for text, up to 100s if it needs to search the web');
